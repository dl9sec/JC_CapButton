...
#ifndef JC_CAPBUTTON_H_INCLUDED
#define JC_CAPBUTTON_H_INCLUDED
...
{
    public:
        // CapButton(pin, dbTime, thHold, invert) instantiates a button object.
        //
        // Required parameter:
        ...
        // thHold   Threshold to detect a button pressed (default 50)
        // invert   true to interpret a low logic level as pressed (default true)
        CapButton(uint8_t pin, uint32_t dbTime=25, uint8_t thHold=50, uint8_t invert=true)
            : m_pin(pin), m_dbTime(dbTime), m_thHold(thHold), m_invert(invert) {}
    ...
    private:
        ...
        uint8_t m_thHold;       // threshold to detect a button pressed
...
class ToggleCapButton : public CapButton
{
    public:
    
        // constructor is similar to Button, but includes the initial state for the toggle.
        ToggleCapButton(uint8_t pin, bool initialState=false, uint32_t dbTime=25, uint8_t thHold=50, uint8_t invert=true)
            : CapButton(pin, dbTime, thHold, invert), m_toggleState(initialState) {}
